...
class MongoDBCollection:
    ...
    def get_collection():
        if not MongoDBCollection.collection:
            ...
            database = client.get_database("databaseProva1")
            MongoDBCollection.collection = database.get_collection("prova1")
            MongoDBCollection.collection.delete_many({})
        return MongoDBCollection.collection


class Neo4jDriver:
    neo4j_host = "neo4j+s://8ae92661.databases.neo4j.io"
    neo4j_user = "neo4j"
    neo4j_password = "Icz1DC58Py8Scwryd9ElTjEUuDhW4W3JPw0uDZEMHQE"
...
class ItemDAO:
    ...
    def get_available_itens(self, capacity, coins):
        # pipeline dedicated to made the query in the mongoDb database
        pipeline = [
            {"$match": {"price": {"$lte": coins}, "weight": {"$lte": capacity}}},
            {"$sort": {"price": 1, "weight": 1}},
...
class CharacterDAO:
    ...
    def add_character(self, character: Character):
        self.neo4j_driver.execute_query(
            "CREATE (character:Character {name: $name, age: $age, habilities: $habilities, profession: $profession, culture: $culture, creation_date: $creation_date})",
            character.to_dict(),
        )
...
def test_questao_2():
    ...
    characters_data = [
        ...
        },
        {
            ...
            "age": 113,
            "habilities": [
                "Manipulação de magia arcana",
                "Estudo de artefatos antigos",
                "Criar golems",
            ],
            "profession": ["Acadêmico", "Canalizador"],
            "culture": ["Eron"],
        ...
        },
        {
            ...
            "age": 29,
            "habilities": [
                "Caça de criaturas das sombras",
                "Sobrevivência no Vale da Morte",
                "Camuflagem",
            ],
    ...
    for character_data in characters_data:
        character = Character(
            character_data["name"],
            character_data["age"],
            character_data["habilities"],
            character_data["profession"],
            character_data["culture"],
            character_data["creation_date"],
        )
